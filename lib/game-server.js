...
class GameServer {
    constructor() {
        ...
        this.rows = 5;
        this.playersNums = 4; // Count Player in room
        this.countDown = 5;
        this.delay = 2000;
    ...
    home() {
        ...
        players.score = 7;
        banker.score = 0;
        this.isCallState(players, banker);
    }
    isCallState(players, banker) {
        console.log(players, banker);
        if (players.score < 6(players.cards.length === 2 && banker.score < 6)) {
        ...
        if (players.cards.length === 3 && banker.cards.length === 2) {
            ...
            if (banker.cards.length === 3)
                banker.score = this.isScores3Cards(banker.cards);
        }
        console.log("players,banker", players, banker);
    }
    isScores(cards) {
        let score = ((((cards[0] % 13) + 1) > 9 ? 0 : ((cards[0] % 13) + 1)) + (((cards[1] % 13) + 1) > 9 ? 0 : ((cards[1] % 13) + 1))) % 10;
        return score;
    }
    isScores3Cards(cards) {
        let score = ((((cards[0] % 13) + 1) > 9 ? 0 : ((cards[0] % 13) + 1)) + (((cards[1] % 13) + 1) > 9 ? 0 : ((cards[1] % 13) + 1)) + (((cards[2] % 13) + 1) > 9 ? 0 : ((cards[2] % 13) + 1))) % 10;
        return score;
    }
    calculator(cards) {
        let card1score = ~~(cards[0] % 13) + 1;
        let card2score = ~~(cards[1] % 13) + 1;
        let aScore = card1score >= 10 ? 0 : card1score;
        let bScore = card2score >= 10 ? 0 : card2score;
        let score = (aScore + bScore) % 10;
        let result = {
            cards: cards,
            multiple: 0,
            score: cards.reduce((p, c) => ((p + (((c % 13) + 1) > 9 ? 0 : ((c % 13) + 1))) % 10), 0),
        };
        return result;
    }
    ...
    calculate(cards) {
        ...
        if (_player.cards.length === 2) {
            ...
            let $cardstype = _player.cards.map((v) => (v % 13)).sort((a, b) => (a - b));
            ...
            else if ((($cardstype[0] + 1 === $cardstype[1]) &&
    ...
    playersSheet() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (let m = 0; m < this.playersNums; m++) {
                ...
                let sessionId = yield this.generateSession();
    ...
    playGames() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let aa1 = -1;
            let vv = [];
            let aa = Array.from(Array(60).keys()).map((v, i) => i + 1);
    ...
    calculatorMaxLines() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let waitCallLucky = yield this.calculatorLucky();
    ...
    countLosesLine() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let findMaxWinLine = yield this.calculatorMaxLines();
            for (let m = 0; m < this.playersNums; m++) {
    ...
    calculatorState() {
        return __awaiter(this, void 0, void 0, function* () {
            let lucky = 2;
            let findMaxWinLine = yield this.calculatorMaxLines();
            let sumtWinslines = yield this.countWinsline();
            let countLosesLines = yield this.countLosesLine();
            for (let m = 0; m < this.playersNums; m++) {
                if (this.players[m].winCountlines === 0) {
